package com.habisofttech.ceronne.testrecyclerview;
...
import com.habisofttech.ceronne.testrecyclerview.data.ProgrammeContract;
                ...
                try{
                    if(isConnected()) {
                        ...
                        for (int i = 0; i < json_programme.length(); i++) {
                            ...
                            value.put(ProgrammeContract.MatiereEntry.COLUMN_MATIERE_LOGO,
                                    json_programme.getJSONObject(i)
                                            .getString(Config.LOGO_KEY));

                            value.put(ProgrammeContract.MatiereEntry.COLUMN_MATIERE_NOM,
                                    json_programme.getJSONObject(i)
                                            .getString(Config.NOM_KEY));

                            value.put(ProgrammeContract.MatiereEntry.COLUMN_MATIERE_DESCRIPTION,
                                    json_programme.getJSONObject(i)
                                            .getString(Config.DESCRIPTION_KEY));
                            values[i] = value;
                        }
                        getContentResolver().bulkInsert(ProgrammeContract.MatiereEntry.CONTENT_URI,values);
                    }
                    return getContentResolver().query(ProgrammeContract.MatiereEntry.CONTENT_URI,
                            null,
                            null,
                            null,
                            null);
